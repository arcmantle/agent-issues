...
type EntityHeadRow = { id: string; reference: string; created_by: string; updated_by: string; kind: string; title: string; body: string; body_source: string; status: string; revision: number; content_hash: string; tombstone: boolean; project_id: string | null; created_at: string; updated_at: string; parent_id: string | null };
...
type ContextTermHeadRow = { id: string; created_by: string; updated_by: string; context_key: string; term: string; definition: string; avoid_terms: string; tombstone: boolean; revision: number; content_hash: string; created_at: string; updated_at: string };
type IssueCommentHeadRow = { id: string; reference: string; issue_id: string; created_by: string; updated_by: string; body: string; tombstone: boolean; revision: number; content_hash: string; created_at: string; updated_at: string };
...
export async function exportCanonicalChains(executor: TenantExecutor): Promise<CanonicalChainBundle> {
	...
	return {
		...
		issueComments: (commentsResult.rows as IssueCommentHeadRow[]).map((row): CanonicalIssueCommentChain => ({ head: { id: row.id, reference: row.reference, issueId: row.issue_id, createdBy: row.created_by, updatedBy: row.updated_by, body: row.body, referencedIssueIds: commentReferenceRows.filter((commentReference) => commentReference.comment_id === row.id).map((commentReference) => commentReference.issue_id), tombstone: row.tombstone, revision: row.revision, contentHash: row.content_hash, createdAt: row.created_at, updatedAt: row.updated_at }, deltas: ledgerRows.filter((delta) => delta.record_kind === "issue-comment" && delta.record_key === encodeIssueCommentRecordKey(row.id)).map(mapDelta) })),
...
async function writeEntity(executor: TenantExecutor, chain: CanonicalEntityChain, created: boolean, projectId: string): Promise<void> {
	...
	if (created) await executor.execute(sql`INSERT INTO entities (tenant_id,id,reference,created_by,updated_by,kind,title,status,body,body_source,revision,content_hash,tombstone,project_id,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.reference},${head.createdBy}::uuid,${head.updatedBy}::uuid,${head.kind},${head.title},${head.status},${head.body},${head.bodySource},${head.revision},${head.contentHash},${head.tombstone},${projectId},${head.createdAt},${head.updatedAt})`);
	else await executor.execute(sql`UPDATE entities SET updated_by=${head.updatedBy}::uuid,title=${head.title},status=${head.status},body=${head.body},body_source=${head.bodySource},revision=${head.revision},content_hash=${head.contentHash},tombstone=${head.tombstone},project_id=${projectId},updated_at=${head.updatedAt} WHERE tenant_id=${executor.tenantId} AND id=${head.id}`);
...
async function writeContext(executor: TenantExecutor, chain: CanonicalContextChain, created: boolean): Promise<void> {
	const head = chain.head;
	if (created) await executor.execute(sql`INSERT INTO contexts (tenant_id,id,reference,key,created_by,updated_by,scope_entity_id,title,summary,revision,content_hash,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.reference},${head.key},${head.createdBy}::uuid,${head.updatedBy}::uuid,${head.scopeEntityId},${head.title},${head.summary},${head.revision},${head.contentHash},${head.createdAt},${head.updatedAt})`);
...
async function writeTerm(executor: TenantExecutor, chain: CanonicalContextTermChain, created: boolean): Promise<void> {
	const head = chain.head;
	if (created) await executor.execute(sql`INSERT INTO context_terms (tenant_id,id,context_key,created_by,updated_by,term,definition,avoid_terms,revision,content_hash,tombstone,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.contextKey},${head.createdBy}::uuid,${head.updatedBy}::uuid,${head.term},${head.definition},${JSON.stringify(head.avoid)},${head.revision},${head.contentHash},${head.tombstone},${head.createdAt},${head.updatedAt})`);
...
async function writeIssueComment(executor: TenantExecutor, chain: CanonicalIssueCommentChain, created: boolean): Promise<void> {
	const head = chain.head;
	if (created) await executor.execute(sql`INSERT INTO issue_comments (tenant_id,id,reference,issue_id,created_by,updated_by,body,revision,content_hash,tombstone,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.reference},${head.issueId}::uuid,${head.createdBy}::uuid,${head.updatedBy}::uuid,${head.body},${head.revision},${head.contentHash},${head.tombstone},${head.createdAt},${head.updatedAt})`);
