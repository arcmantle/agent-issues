...
async function writeContext(executor: TenantExecutor, chain: CanonicalContextChain, created: boolean): Promise<void> {
	const head = chain.head;
	if (created) await executor.execute(sql`INSERT INTO contexts (tenant_id,id,reference,key,scope_entity_id,title,summary,revision,content_hash,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.reference},${head.key},${head.scopeEntityId},${head.title},${head.summary},${head.revision},${head.contentHash},${head.createdAt},${head.updatedAt})`);
	else await executor.execute(sql`UPDATE contexts SET scope_entity_id=${head.scopeEntityId},title=${head.title},summary=${head.summary},revision=${head.revision},content_hash=${head.contentHash},updated_at=${head.updatedAt} WHERE tenant_id=${executor.tenantId} AND key=${head.key}`);
...
async function writeTerm(executor: TenantExecutor, chain: CanonicalContextTermChain, created: boolean): Promise<void> {
	const head = chain.head;
	if (created) await executor.execute(sql`INSERT INTO context_terms (tenant_id,id,context_key,term,definition,avoid_terms,revision,content_hash,tombstone,created_at,updated_at) VALUES (${executor.tenantId},${head.id}::uuid,${head.contextKey},${head.term},${head.definition},${JSON.stringify(head.avoid)},${head.revision},${head.contentHash},${head.tombstone},${head.createdAt},${head.updatedAt})`);
	else await executor.execute(sql`UPDATE context_terms SET context_key=${head.contextKey},term=${head.term},definition=${head.definition},avoid_terms=${JSON.stringify(head.avoid)},revision=${head.revision},content_hash=${head.contentHash},tombstone=${head.tombstone},updated_at=${head.updatedAt} WHERE tenant_id=${executor.tenantId} AND id=${head.id}::uuid`);
