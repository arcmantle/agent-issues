 ...
 * `0008-consolidate-legacy-tenants-backfill.ts`), which may differ from
 * whichever tenant this open's own bootstrap trio just seeded counters for
 * (an explicit `--tenant` open bootstraps ITS OWN counters, not the
 * well-known tenant's) - `mintEntityId`/`mintHandoffId` below need the
 * well-known tenant's counters to already exist regardless. Idempotent
 * (`ON CONFLICT ... DO NOTHING`) and cheap, so always safe to run
 * unconditionally at the top of this migration.
 */
 ...
 * `buildConsolidateLegacyTenantMigration` (a single-tenant invocation
 * helper, kept for this migration's own dedicated test coverage) and
 ...
 * first runs, ISS181 - the only production caller now that consolidation is
 * migration-only) can reuse the identical body without duplicating it.
 */
 ...
 * Consolidation is migration-only (there is no manual/on-demand admin
 * path) - the automatic historical sweep instead runs every outstanding
 * legacy tenant through `consolidateLegacyTenantData` directly inside one
 * fixed-id migration (`migrations/0008-consolidate-legacy-tenants-backfill.ts`,
 * ISS181). This builder's only current caller is this file's own test
 * suite, exercising `consolidateLegacyTenantData`'s full remap behavior for
 * a single tenant without duplicating 0008's fixed-id migration shape.
 */
