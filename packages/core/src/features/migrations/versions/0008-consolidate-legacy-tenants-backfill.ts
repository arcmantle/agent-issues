 ...
 * forward - it is never automatically swept into a project. There is no
 * manual/on-demand way to fold such a tenant in later: consolidation is
 * migration-only, by design (a tenant is never "wrongly" created regardless
 * of version, since every workspace with no explicit `--tenant` flag always
 * resolves to the well-known tenant automatically).
 */
