...
1. List unfinished issues with `agent-issues list issue --status 'todo,in-progress,blocked' --parent <initiativeId> --json`. Quote comma-separated option values so PowerShell passes them as one argument. Each item carries its `reference`, which is the key used by `openBlockers`. Its compact JSON also includes `openBlockers`: a reference -> open, not-`done`, blocking issue references map, computed for every issue in the response.
