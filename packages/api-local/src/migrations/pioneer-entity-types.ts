...
async function rewriteEntityHistory(conn: Parameters<Migration["up"]>[0], entity: EntityRow): Promise<void> {
	...
	let successor = {
		...
		tombstone: entity.tombstone === true || entity.tombstone === 1
	};
...
export const pioneerEntityTypesMigration: Migration = {
	...
	async up(conn) {
		if (conn.dialect !== "sqlite") {
			throw new Error("SQLite Pioneer entity type migration requires the SQLite dialect.");
		}
				...
				AND (
					...
					OR EXISTS (
						...
						WHERE revision.tenant_id = entity.tenant_id
							...
							AND revision.record_key = length(CAST(entity.id AS BLOB)) || ':' || entity.id
							AND instr(revision.reverse_patch, CAST(${"wayfinder-"} AS BLOB)) > 0
					)
				)
		`);
