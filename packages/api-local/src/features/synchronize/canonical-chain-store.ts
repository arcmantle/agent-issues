...
function writeContext(db: SqliteExecutor, chain: CanonicalContextChain, created: boolean): void {
	...
	const projectId = getContextProjectId(db, head.key, head.scopeEntityId);
...
function writeContextTerm(db: SqliteExecutor, chain: CanonicalContextTermChain, created: boolean): void {
	const head = chain.head;
	if (created) db.drizzle.run(sql`INSERT INTO context_terms (tenant_id,id,context_key,term,definition,avoid_terms,revision,content_hash,tombstone,created_at,updated_at) VALUES (${db.tenantId},${head.id},${head.contextKey},${head.term},${head.definition},${JSON.stringify(head.avoid)},${head.revision},${head.contentHash},${head.tombstone ? 1 : 0},${head.createdAt},${head.updatedAt})`);
	else db.drizzle.run(sql`UPDATE context_terms SET context_key=${head.contextKey},term=${head.term},definition=${head.definition},avoid_terms=${JSON.stringify(head.avoid)},revision=${head.revision},content_hash=${head.contentHash},tombstone=${head.tombstone ? 1 : 0},updated_at=${head.updatedAt} WHERE tenant_id=${db.tenantId} AND id=${head.id}`);
	const projectId = getContextProjectId(db, head.contextKey);
...
function getEntityProjectId(db: SqliteExecutor, entityId: string): string {
	const row = db.drizzle.all(sql`SELECT project_id FROM entities WHERE tenant_id = ${db.tenantId} AND id = ${entityId}`)[0] as { project_id: string | null } | undefined;
	if (!row?.project_id) throw new Error(`Cannot resolve project for entity ${entityId}.`);
	return row.project_id;
}

function getContextProjectId(db: SqliteExecutor, contextKey: string, scopeEntityId?: string | null): string {
	if (scopeEntityId) return getEntityProjectId(db, scopeEntityId);
	...
	const row = db.drizzle.all(sql`SELECT scope_entity_id FROM contexts WHERE tenant_id = ${db.tenantId} AND key = ${contextKey}`)[0] as { scope_entity_id: string | null } | undefined;
	if (row?.scope_entity_id) return getEntityProjectId(db, row.scope_entity_id);
